...
BEGIN
  ...
  SELECT
    ar.id,
    ar.staff_id,
    COALESCE(p.full_name, 'Unknown staff member'),
    ar.date,
    'photos'::text,
    ARRAY_REMOVE(ARRAY[ar.clock_in_photo_url, ar.clock_out_photo_url], NULL),
    hs.photo_retention_days
  ...
  SELECT
    ...
    'location'::text,
    ARRAY[]::text[],
    hs.location_retention_days
  FROM public.attendance_records ar
  JOIN public.staff_hr_settings hs ON hs.staff_id = ar.staff_id
  LEFT JOIN public.profiles p ON p.id = ar.staff_id
  WHERE (p_staff_id IS NULL OR ar.staff_id = p_staff_id)
    AND hs.store_location
    AND ar.date < CURRENT_DATE - hs.location_retention_days
    AND (
      ar.clock_in_latitude IS NOT NULL OR ar.clock_in_longitude IS NOT NULL
      OR ar.clock_out_latitude IS NOT NULL OR ar.clock_out_longitude IS NOT NULL
    );
END;
$$;

REVOKE ALL ON FUNCTION public.get_attendance_retention_candidates(uuid) FROM PUBLIC;
...
BEGIN
  ...
  IF p_data_type = 'photos' THEN
    ...
    SELECT e.id, e.in_path, e.out_path
    FROM eligible e
    JOIN cleared c ON c.id = e.id;
  ELSE
    RETURN QUERY
    WITH eligible AS (
      SELECT ar.id, ar.clock_in_photo_url AS in_path, ar.clock_out_photo_url AS out_path
...
REVOKE ALL ON FUNCTION public.clear_attendance_retention_data(uuid[], text) FROM PUBLIC;
