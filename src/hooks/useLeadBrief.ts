...
export function useLeadBrief(
  { leadId, taskId }: { leadId?: string | null; taskId?: string | null },
  enabled = true,
) {
  ...
  const targetKey = `${leadId ?? ""}:${taskId ?? ""}`;

  const load = useCallback(async () => {
    const requestId = ++requestIdRef.current;
    const isCurrent = () => requestId === requestIdRef.current;

    if (!enabled || (!leadId && !taskId)) {
      setLead(null);
      setProfessionals([]);
      setTrail([]);
      setLoadedKey(null);
    ...
    try {
      ...
      if (!resolvedLeadId && taskId) {
        const { data: taskContext, error } = await supabase
          .from("tasks")
        ...
        resolvedLeadId =
          taskContext?.lead_id ||
          (taskContext?.related_entity_type === "lead"
            ? taskContext.related_entity_id
            : null);

        if (taskContext?.related_entity_type === "professional") {
          resolvedProfessionalId = taskContext.related_entity_id;
        }
      }

      let directProfessional: Awaited<ReturnType<typeof resolveProfessionalById>> = null;
      ...
      if (!resolvedLeadId) {
        ...
        setProfessionals(
          directProfessional
            ? [
                {
                  id: directProfessional.id,
                  name: directProfessional.name,
                  phone: directProfessional.phone,
                  designation: directProfessional.professional_type,
                },
              ]
            : [],
        );
      ...
      const [leadResult, profResult, taskResult, legacyTaskResult] = await Promise.all([
        ...
        supabase
          .from("lead_professionals")
          .select(
            "professional_id,is_primary_contact,contact_designation,professionals(id,name,phone,professional_type)",
          )
      ...
      for (const row of (profResult.data || []) as any[]) {
        ...
        professionalMap.set(row.professionals.id, {
          id: row.professionals.id,
          name: row.professionals.name,
          phone: row.professionals.phone || null,
          designation:
            row.contact_designation || row.professionals.professional_type || null,
        });
      }

      if (directProfessional) {
        professionalMap.set(directProfessional.id, {
          id: directProfessional.id,
          name: directProfessional.name,
          phone: directProfessional.phone,
          designation: directProfessional.professional_type,
      ...
      for (const item of [...(taskResult.data || []), ...(legacyTaskResult.data || [])]) {
      ...
      for (const task of tasks) {
        const isCompleted = task.status === "Completed" || Boolean(task.completed_at);
        const isRescheduled = Boolean(task.reschedule_reason?.trim());

        if (isCompleted && task.completion_notes?.trim()) {
          addEntry({
  ...
  }, [enabled, leadId, taskId, targetKey]);
