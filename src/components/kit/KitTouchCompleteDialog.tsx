...
interface KitTouchCompleteDialogProps {
  ...
  onComplete: (outcome: string, notes?: string, alsoCompleteTask?: boolean) => Promise<void>;
...
export function KitTouchCompleteDialog({
  ...
  const [alsoCompleteTask, setAlsoCompleteTask] = useState<boolean>(true);

  // Reset toggle whenever dialog opens
  useEffect(() => {
    if (open) setAlsoCompleteTask(true);
  }, [open]);
  ...
  return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md z-[120]" hideOverlay>
        <DialogHeader>
          ...
          {hasLinkedTask && (
            <div className="flex items-start space-x-3 p-3 rounded-md border bg-muted/30">
              <Checkbox
                ...
                checked={alsoCompleteTask}
                onCheckedChange={(checked) => setAlsoCompleteTask(checked === true)}
                className="mt-0.5"
              />
              <div className="space-y-0.5">
                <Label htmlFor="also-complete-task" className="cursor-pointer font-medium">
                  Also mark the linked task as Completed
                </Label>
                <p className="text-xs text-muted-foreground">
                  The task linked to this touch will be closed automatically
                </p>
              </div>
            </div>
          )}
