...
interface CustomerSavedFilterDialogProps {
  open: boolean;
...
const FIELD_OPTIONS = [
  ...
  { value: "pending_tasks_count", label: "Pending Tasks Count", type: "number", category: "Tasks" },
    ...
    switch (field) {
      ...
      case "pending_tasks":
        return [
          ...
          { value: "no_pending", label: "No Pending Tasks" },
          { value: "has_overdue", label: "Has Overdue Tasks" },
          { value: "due_today", label: "Tasks Due Today" },
        ];
      default:
        return [];
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Create filter rules for customers.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4 flex-1 overflow-y-auto">
                  ...
                  <div className="flex justify-center">
                    ...
                    >
                      <div className="flex items-center space-x-1">
                        <RadioGroupItem value="and" id={`cand-${index}`} className="h-3 w-3" />
                        <Label htmlFor={`cand-${index}`} className="text-xs font-medium cursor-pointer">AND</Label>
                      </div>
                      <div className="flex items-center space-x-1">
                        <RadioGroupItem value="or" id={`cor-${index}`} className="h-3 w-3" />
                        <Label htmlFor={`cor-${index}`} className="text-xs font-medium cursor-pointer">OR</Label>
                      </div>
                    </RadioGroup>
                  </div>
                )}
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch id="customerIsShared" checked={isShared} onCheckedChange={setIsShared} />
            </div>
            <div className="flex items-center justify-between">
              <Label htmlFor="customerIsDefault">Set as default filter</Label>
              <Switch id="customerIsDefault" checked={isDefault} onCheckedChange={setIsDefault} />
            </div>
          </div>
        </div>
