        ...
        <>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              {phone && <PhoneLink phone={phone} log={leadId ? { relatedEntityType: "lead", relatedEntityId: leadId } : undefined} />}
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            {alternatePhone && <PhoneLink phone={alternatePhone} log={leadId ? { relatedEntityType: "lead", relatedEntityId: leadId } : undefined} />}
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {contacts.slice(0, 4).map((contact, index) => (
            <div key={`${contact.name || "contact"}-${contact.phone || index}`} className="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs">
              ...
              {contact.phone ? <PhoneLink phone={contact.phone} log={leadId ? { relatedEntityType: "lead", relatedEntityId: leadId } : undefined} /> : <Phone className="h-3 w-3 text-muted-foreground" />}
            </div>
          ))}
        </div>
      )}
