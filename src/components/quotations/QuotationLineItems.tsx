...
export function QuotationLineItems({ items, onChange }: QuotationLineItemsProps) {
  const addItem = () => onChange([...items, { id: `temp-${Date.now()}-${Math.random()}`, item_name: '', quantity: 1, unit: 'pcs', rate: 0, amount: 0, sort_order: items.length }]);
  ...
  const duplicateItem = (index: number) => onChange([...items, { ...items[index], id: `temp-${Date.now()}-${Math.random()}`, sort_order: items.length }]);
